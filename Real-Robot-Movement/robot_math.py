...
class RobotMath():
    ...
    def platform_inverse_kinematics(self, x, y, z, roll, pitch, yaw, radius, angle, l1, l2, l3, initialX, initialZ):
        ...
        S1 = np.array([ 
            ...
            [radius * math.cos(angle + math.pi / 3), radius * math.sin(angle + math.pi / 3), 0 ],
            [radius * math.cos(angle + 2 * math.pi / 3), radius * math.sin (angle + 2 * math.pi / 3), 0 ],
            [radius * math.cos(angle + math.pi), radius * math.sin(angle + math.pi), 0 ],
            [radius * math.cos(angle + 4 * math.pi / 3), radius * math.sin(angle + 4 * math.pi / 3), 0 ],
            [radius * math.cos(angle + 5 * math.pi / 3), radius * math.sin(angle + 5 * math.pi / 3), 0 ]
            ])

        S1 = np.transpose(S1)

        U = np.array([
            [(initialX + radius) *   math.cos(angle), (initialX + radius) *   math.sin(angle), initialZ],
            [(initialX + radius) *   math.cos(angle + math.pi / 3), (initialX + radius) *   math.sin(angle + math.pi / 3), initialZ],
            [(initialX + radius) *   math.cos(angle + 2 * math.pi / 3), (initialX + radius) *   math.sin (angle + 2 * math.pi / 3), initialZ],
            [(initialX + radius) *   math.cos(angle + math.pi), (initialX + radius) *   math.sin(angle + math.pi), initialZ],
            [(initialX + radius) *   math.cos(angle + 4 * math.pi / 3), (initialX + radius) *   math.sin(angle + 4 * math.pi / 3), initialZ],
            [(initialX + radius) *   math.cos(angle + 5 * math.pi / 3), (initialX + radius) *   math.sin(angle + 5 * math.pi / 3), initialZ]
        ])
        U = np.transpose(U)
        ...
        for i in range(6):
            ...
            beta[0, i] = math.acos((l2 ** 2 - l3 ** 2 + LP[0, i] ** 2) / (2 * l2 * LP[0, i])) - (p + phi)
            gamma[0, i] = math.pi - math.acos((l2 ** 2 + l3 ** 2 - LP[0, i] ** 2) / (2 * l2 * l3)) * -1.0
